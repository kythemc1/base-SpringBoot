...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public JWTAuthenticationFilter(AuthenticationManager authenticationManager, BCryptPasswordEncoder bCryptPasswordEncoder) {
        ...
        // ログイン用pathの設定
        setRequiresAuthenticationRequestMatcher(new AntPathRequestMatcher(Commons.LOGIN_URL, "POST"));
    }
                                            ...
                                            Authentication auth) throws IOException, ServletException
    {
        ...
        try {
            String token = Jwts.builder()
                    ...
                    .signWith(SignatureAlgorithm.HS512, Commons.TOKEN_SECRET.getBytes())
                    .compact();
            res.addHeader(Commons.AUTH_HEADER, Commons.TOKEN_PREFIX + token);
        }catch (Throwable e){
            logger.error(e);
        }
    }
